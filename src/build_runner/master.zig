        ...
        for (compile.root_module.include_dirs.items) |include_dir| {
            switch (include_dir) {
                ...
                .other_step => |other| {
                    if (other.generated_h) |header| {
                        try include_dirs.put(
                            arena,
                            std.fs.path.dirname(header.path.?).?,
                            {},
                        );
                    }
                    if (other.installed_headers.items.len > 0) {
                        try include_dirs.put(
                            arena,
                            owner.pathJoin(&.{ other.step.owner.install_prefix, "include" }),
                            {},
                        );
                    }
                },
                .config_header_step => |config_header| {
                    ...
                    try include_dirs.put(
                        arena,
                        full_file_path[0 .. full_file_path.len - config_header.include_path.len],
                        {},
                    );
                },
            }
